...
- 当你说 `123` 时，表示：先升级版本号，然后更新 `README.md`，再打包、提交并推送
- 默认理解为：按这个顺序完成当前这轮变更的收尾工作
- 如果某次你只想执行其中一部分，需要单独明确说明

当前这台机器默认作为主工作机使用：

- 后续主要在这台机器上改代码、打包和推送
- 日常流程只走 `main`，不再新建分支
- 当前 `.venv` 已切换为本机 Python 环境，后续优先直接使用它
- 旧的 `.venv_old/` 只是本地备份，不参与仓库和打包
...
`v0.6.121` 这一轮版本内容比较集中，重点新增和调整如下：

- 订单编号前缀统一整理：
  - 策略、套利、智能下单、风控保护和会话相关下单都开始统一走程序前缀
  - 便于后续在成交记录、日志和界面里稳定识别订单归属
  - 新增并补齐了相关单元测试

- 止损执行归因链路补齐：
  - 新增 `stop_execution.py`，统一评估计划风险、实际亏损、止损滑点和超额幅度
  - 止损触发后的通知、对账记录和会话详情现在都会带上归因等级和关键数值
  - 新增单元测试，覆盖真实止损案例和动态止损偏移场景

- 期权策略界面的紧凑数字显示统一收窄：
  - `_format_compact_number` 现在最多保留 4 位小数，再去掉末尾多余的 0 和小数点
  - 小数展示更短，适合期权报价、行权价和细粒度数值的界面阅读
  - 对应新增了单元测试，覆盖 4 位小数以内的常见格式
...
- 策略对账后的半自动任务结算修正：
  - 对账完成后会显式走主应用的半自动任务结算实现
  - 这样可以更稳定地把关联任务推进到 `completed_closed`
- 新增 `EMA55 斜率做空` 策略：
...
- Qt 端这轮开始从“单一终端”往“多模块主壳”升级：
  ...
  - 当前主壳保留 `专业套利终端` 与 `无限下单`；`划线交易台` 不再作为独立入口，`自动通道` 后续并入 K 线功能
...
- Qt 本地交易终端现已采用单窗口实时架构：
  - 主窗口默认打开 K线，并在全局顶栏提供“K线 / 持仓 / 专业套利 / 交易工具 / 期权工具”；账户持仓和专业套利按需创建，返回后保留页面现场
  - 新启动默认优先使用 `moni` API 配置；没有 `moni` 时才回退到已选或首个可用配置
  - API、模拟/实盘环境、行情/私有连接状态和本地任务摘要统一提升到全局顶栏；页面内不再重复显示 API 下拉框
  - 顶部 API 只决定当前查看账户和新操作账户；已运行 RR、线条条件和自动套利继续绑定启动时的 API，不会因查看其他账户而改发订单
  - K线图默认显示，EMA15/SMA50 默认开启，1H/4H/1D 形态默认关闭；嵌入页使用紧凑的“形态：关/开”菜单
  - 自动通道已收进 K线左侧分析模块：自动箱体、历史箱体、通道显示与箱体突破告警彼此独立，默认关闭；通道识别复用原研究模块，参数按 API / 标的 / 周期保存
  - K线页仍可隐藏图表，RR 与本地条件任务不会因切页或隐藏图表而停止
  - `orders`、`positions`、`account` 使用私有 WS，条件/OCO 类算法单使用业务 WS `orders-algo`；REST 仅用于启动快照、断线恢复、60 秒安全校验与历史查询
  - 当前合约 K线在首次历史加载后订阅业务 WS；开 K 替换、新 K 追加都走增量图表更新，不重新拉取整段历史
  - Qt 主进程不再创建隐藏 Tk 根窗口或运行 Tk 事件泵；遗留 Tk 期权工具会独立启动，并继续使用同一数据目录
...
- 下单安全这轮新增了绝对风控线：
  - `okx_quant/okx_client.py` 下单前加入名义规模拦截：BTC 方向默认上限 1 BTC，非 BTC 方向默认上限 5000 美元等值
  - 当合约参数不完整无法精确估算风险时，改为请求现货/指数最新价格作为兜底，不确定时直接阻断下单，避免超量挂单直达交易所
- 期权保护链路这轮补了“启动风控”和“成交收敛”：
  - `okx_quant/ui_shell.py` 启动前会检查保护触发价与当前触发价关系，避免提交后立即触发
  - `okx_quant/position_protection.py` 记录平仓成交时增加成交单去重与累计逻辑，避免重复补单导致的超额报错
  - 成交等待改为更稳态：从 partial fill 继续走到终态，再对未成交通知补充成交量信息（包括超时/取消时可能产生的已成交）
- 回测区新增“纯本地回测”链路：
...
- [okx_quant/strategy_symbol_defaults.py](/D:/qqokx/okx_quant/strategy_symbol_defaults.py)：分币种策略默认模板，当前已固化 `v0.6.95` 的多头/空头独立参数
...
- [版本开发日志_v0.6.88.md](/D:/qqokx/版本开发日志_v0.6.88.md)

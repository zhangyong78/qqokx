...
def btc_ema15_ma50_pullback_short_minimum_candles(config: StrategyConfig) -> int:
    return max(
...
def btc_ema15_ma50_pullback_short_bias_allows_short(
...
def scan_btc_ema15_ma50_pullback_short_candidates(
    candles: list[Candle],
    ...
    *,
    ...
    for index in range(1, len(candles)):
        ...
        if is_cross_down(fast_values, slow_values, index):
        ...
        if candle.high < fast_value or candle.close >= fast_value:
            continue

        active_pullback_count += 1
        daily_filter_pass = btc_ema15_ma50_pullback_short_bias_allows_short(direction_filter_bias, index)
        pullback_depth_pct = (
            ((candle.high - fast_value) / fast_value) * Decimal("100")
            if candle.high > fast_value and fast_value > 0
            else Decimal("0")
        )
        candidates.append(
            PullbackCandidate(
...
def evaluate_btc_ema15_ma50_pullback_short_signal(
    candles: list[Candle],
    config: StrategyConfig,
    *,
    direction_filter_bias: list[str] | None = None,
    ...
    if latest_candidate is None:
        ...
        return SignalDecision(
            signal=None,
            reason="btc_ema15_ma50_pullback_short_not_ready",
    ...
    if not latest_candidate.daily_filter_pass:
        return SignalDecision(
            signal=None,
            reason="daily_filter_blocks_short",
            candle_ts=latest_candidate.signal_ts,
            entry_reference=None,
            atr_value=latest_candidate.atr_at_signal,
            ema_value=latest_candidate.ema15_at_signal,
    ...
    return SignalDecision(
        signal="short",
        reason=(
            f"crossdown_pullback_short_ready: close={px(latest_candle.close)} "

...
VOL_SIGNAL_UI_LABELS: dict[VolatilitySignalType, str] = {
...
VOL_SIGNAL_DEFINITIONS: tuple[_VolatilitySignalDefinition, ...] = (
    _VolatilitySignalDefinition("bearish_reversal_after_rally", VOL_SIGNAL_UI_LABELS["bearish_reversal_after_rally"]),
    _VolatilitySignalDefinition("bullish_reversal_after_drop", VOL_SIGNAL_UI_LABELS["bullish_reversal_after_drop"]),
    _VolatilitySignalDefinition("squeeze_breakout_up", VOL_SIGNAL_UI_LABELS["squeeze_breakout_up"]),
    _VolatilitySignalDefinition("squeeze_breakout_down", VOL_SIGNAL_UI_LABELS["squeeze_breakout_down"]),
    _VolatilitySignalDefinition("box_breakout_up", VOL_SIGNAL_UI_LABELS["box_breakout_up"]),
    _VolatilitySignalDefinition("box_breakout_down", VOL_SIGNAL_UI_LABELS["box_breakout_down"]),
    _VolatilitySignalDefinition("ema34_turn_up", VOL_SIGNAL_UI_LABELS["ema34_turn_up"]),
    _VolatilitySignalDefinition("ema34_turn_down", VOL_SIGNAL_UI_LABELS["ema34_turn_down"]),
)
...
class DeribitVolatilityMonitorWindow:
    def __init__(
        ...
        self._signal_chart_hint = StringVar(
            value="橙色标记为当前信号，蓝色为向上历史信号，紫色为向下历史信号；支持滚轮缩放、左键拖动和平移、悬浮十字光标。"
        )
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Checkbutton(currency_box, text="BTC DVOL", variable=self.enable_btc).grid(
        ...
        ttk.Entry(base_frame, textvariable=self.cumulative_change_threshold).grid(
            row=8, column=0, sticky="ew", pady=(6, 0), padx=(0, 12)
        )
        ...
        ttk.Entry(base_frame, textvariable=self.breakout_body_multiplier).grid(
            row=10, column=0, sticky="ew", pady=(6, 0), padx=(0, 12)
        )
    ...
    def _parse_signal_chart_candle_limit(self) -> int:
        ...
        if value > MAX_VOL_SIGNAL_CHART_CANDLE_LIMIT:
            raise ValueError(f"K线数量不能超过 {MAX_VOL_SIGNAL_CHART_CANDLE_LIMIT}")
        return value
    ...
    def start(self) -> None:
        ...
        self._log(
            session_id,
            f"任务已启动 | 周期={_resolution_label(config.resolution)} | 币种={', '.join(config.currencies)}",
        )
        self._refresh_task_views()
    ...
    def _build_email_sender(
        ...
        def _sender(event: VolatilitySignalEvent, resolution: str) -> None:
            signal_label = VOL_SIGNAL_UI_LABELS.get(event.signal_type, event.signal_type)
            subject = f"[QQOKX] 波动率信号 | {session_id} | {_resolution_label(resolution)} | {event.symbol} | {signal_label}"
            body = "\n".join(
                [
                    "模块：Deribit波动率监控",
                    f"任务：{session_id}",
                    ...
                    f"标的：{event.symbol}",
                    f"信号：{signal_label}",
                    f"方向：{VOL_DIRECTION_UI_LABELS.get(event.direction, event.direction)}",
    ...
    def _build_config(self) -> VolatilityMonitorConfig:
        ...
        if not any(variable.get() for variable in self._signal_enabled_vars.values()):
        ...
        return VolatilityMonitorConfig(
            ...
            cumulative_change_threshold=self._parse_positive_decimal(
                self.cumulative_change_threshold.get(),
                "连续涨跌阈值",
            ),
    ...
    def _append_event(self, session_id: str, resolution: str, event: VolatilitySignalEvent) -> None:
        ...
        values = (
            ...
            event.symbol,
            VOL_SIGNAL_UI_LABELS.get(event.signal_type, event.signal_type),
            VOL_DIRECTION_UI_LABELS.get(event.direction, event.direction),
        ...
        for extra_item_id in children[300:]:
            self._signal_row_payloads.pop(extra_item_id, None)
            self.signal_tree.delete(extra_item_id)
    ...
    def _open_signal_chart(self, request: _VolatilityChartRequest) -> None:
        ...
        signal_label = VOL_SIGNAL_UI_LABELS.get(request.signal_type, request.signal_type)
        if request.mode == "preview":
            self._signal_chart_summary.set(
                f"正在加载 {request.currency} DVOL {_resolution_label(request.resolution)} 的 {request.candle_limit} 根 K 线，并预览 {signal_label} 的历史触发位置..."
            )
        else:
            self._signal_chart_summary.set(
                f"正在加载 {request.currency} DVOL {_resolution_label(request.resolution)} 的 {request.candle_limit} 根 K 线，并回放 {signal_label} 历史信号..."
            )
    ...
    def _ensure_signal_chart_window(self) -> None:
        ...
        chart_window.title("波动率信号 K 线预览")
        apply_adaptive_window_geometry(
            chart_window,
            width_ratio=0.88,
            height_ratio=0.78,
    ...
    def _refresh_active_signal_chart(self) -> None:
        ...
        try:
            if self._signal_chart_active_request.mode == "preview":
                ...
                request = _VolatilityChartRequest(
                    mode="event",
                    currency=active_request.currency,
    ...
    def _clear_signal_chart_hover(self, canvas: Canvas) -> None:
        self._signal_chart_hover_index = None
        self._signal_chart_hover_y = None
        canvas.delete("vol-signal-chart-hover")
    ...
    def _apply_signal_chart_dataset(self, request_id: int, dataset: _VolatilityChartDataset) -> None:
        ...
        summary = (
            f"标的: {request.currency} DVOL | 周期: {_resolution_label(request.resolution)} | 规则: {signal_label} | "
            f"当前加载 K 线: {len(dataset.candles)} 根 | 历史同规则信号: {history_count} 个"
        )
        if request.selected_event is not None:
            selected_found = any(_volatility_events_match(item, request.selected_event) for item in dataset.history_events)
            summary = (
                f"{summary} | 当前信号时间: "
                f"{datetime.fromtimestamp(request.selected_event.candle_ts / 1000).strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    def _draw_signal_chart(
        ...
        *,
        ...
        visible_ema = list(dataset.ema_values[start_index:end_index])
        ...
        plotted_prices.extend(float(item) for item in visible_ema)
        ...
        canvas.create_text(
            ...
            top - 6,
            text=(
                f"显示 {start_index + 1}-{min(end_index, len(candles))}/{len(candles)} "
                "| 滚轮缩放 | 左键拖动 | 悬浮十字光标"
            ),
            anchor="sw",
            fill="#57606a",
        ...
        for index, candle in enumerate(visible_candles, start=start_index):
            ...
            color = _volatility_chart_candle_color(candle.open, candle.close)
        ...
        if len(visible_ema) >= 2:
            ema_points: list[float] = []
            for index, value in enumerate(visible_ema, start=start_index):
        ...
        legend_lines.append("蓝色=向上历史")
        legend_lines.append("紫色=向下历史")
        canvas.create_text(
            width - right,
            top + 10,
...
def _build_volatility_chart_ema_series(
    ...
    if not closes:
        return None, []
    return f"EMA{config.ema_period}", ema(closes, config.ema_period)
...
def _decimal_places_for_volatility_candles(candles: tuple[DeribitVolatilityCandle, ...] | list[DeribitVolatilityCandle]) -> int:
    places = 0
    for candle in candles:
        for value in (candle.open, candle.high, candle.low, candle.close):
            places = max(places, max(-value.as_tuple().exponent, 0))
    return min(max(places, 2), 8)
...
def _format_volatility_chart_price(value: Decimal, decimal_places: int) -> str:
    return format_decimal_fixed(value, decimal_places)


def _volatility_chart_candle_color(open_price: Decimal, close_price: Decimal) -> str:
...
def _volatility_chart_time_label_indices(start_index: int, end_index: int, *, target_labels: int = 6) -> list[int]:
    visible_count = max(end_index - start_index, 0)
    if visible_count <= 0:
        return []
    if visible_count <= target_labels:
        return list(range(start_index, end_index))
    step = max(1, round((visible_count - 1) / max(target_labels - 1, 1)))
    indices = list(range(start_index, end_index, step))
    if indices[-1] != end_index - 1:
...
def _volatility_chart_price_for_y(y: float, state: _VolatilityChartRenderState) -> Decimal:
    usable_height = max(state.height - state.top - state.bottom, 1)
    ratio = min(max((y - state.top) / usable_height, 0.0), 1.0)
    return state.price_max - ((state.price_max - state.price_min) * Decimal(str(ratio)))
...
def _format_volatility_chart_hover_lines(
    *,
    ...
    lines = [
        f"时间: {_format_volatility_chart_timestamp(candle.ts)}",
        (
            "O/H/L/C: "
    ...
    if hover_events:
        signal_text = ", ".join(
            f"{VOL_SIGNAL_UI_LABELS.get(item.signal_type, item.signal_type)}/{VOL_DIRECTION_UI_LABELS.get(item.direction, item.direction)}"

...
from okx_quant.arbitrage.launch_support import (
    build_runtime_for_profile as _shared_build_runtime_for_profile,
    future_expiry_code as _shared_future_expiry_code,
    future_family_key as _shared_future_family_key,
    roll_target_future_candidates as _shared_roll_target_future_candidates,
)
...
ROLL_POSITION_FETCH_TIMEOUT_SECONDS = 20.0
ROLL_OVERVIEW_FETCH_TIMEOUT_SECONDS = 4.0
...
def _estimated_dual_leg_fee_pct(
    *,
    ...
    if panel_key == "roll":
        ...
        if execution_mode == "both_maker_first_taker":
            return (profile.swap_maker + profile.swap_taker) * Decimal("100")
...
def _market_panel_fee_labels_and_liquidity(
    *,
    ...
    if panel_key == "roll":
        ...
        if execution_mode == "both_maker_first_taker":
            return ("旧合约先挂单", "maker"), ("新合约先挂单", "maker")
...
def _build_spot_positions_from_account(overview, client, *, validate_instrument: bool = True) -> list[OkxPosition]:
...
def _future_family_key(inst_id: str) -> str | None:
    return _shared_future_family_key(inst_id)


def _future_expiry_code(inst_id: str) -> str | None:
    return _shared_future_expiry_code(inst_id)


def _roll_target_future_candidates(current_inst_id: str, instruments: list[Instrument]) -> list[str]:
    return _shared_roll_target_future_candidates(current_inst_id, instruments)
...
def _build_runtime_for_profile(
    ...
    *,
    ...
    return _shared_build_runtime_for_profile(
        profile_name,
...
class ArbitrageWindow:
    def __init__(
        ...
        *,
        ...
        self._market_panel_job_due_at: float | None = None
        self._market_panel_refresh_busy = False
        self._market_panel_refresh_token = 0
        self._market_panel_wait_token = 0
        self._roll_preview_refresh_token = 0
        ...
        self._pair_close_positions_refresh_busy = False
        self._pair_close_positions_refresh_requested = False
        self._roll_positions_refresh_busy = False
        self._roll_positions_refresh_requested = False
        self._last_auto_monitor_result_key: tuple[str, int] | None = None
        ...
        self.auto_refresh_enabled = BooleanVar(value=False if fast_mode else bool(settings.get("auto_refresh_enabled", True)))
        ...
        self.monitor_status_text = StringVar(value="未监控")
        self.window = Toplevel(parent)
        self.window.title("现货套利极速版" if self._fast_mode else "现货套利")
        ...
        if self._fast_mode and self._roll_tab is not None:
    ...
    def _safe_after(self, callback: Callable[[], object], delay_ms: int = 0) -> str | None:
        ...
        if getattr(self, "_destroying", False) or not self._widget_exists(window):
            return None
        try:
            return window.after(delay_ms, callback)
        except Exception:
            return None
    ...
    def destroy(self) -> None:
        ...
        self._market_panel_wait_token += 1
        ...
        if self._market_panel_job is not None:
            ...
            self._market_panel_job_due_at = None
    ...
    def _build_layout(self) -> None:
        ...
        header_title = "现货套利 极速移仓" if self._fast_mode else "现货套利 V1.1"
        ttk.Label(header, text=header_title, font=("Microsoft YaHei UI", 16, "bold")).grid(row=0, column=0, sticky="w")
    ...
    def _build_roll_tab(self, notebook: ttk.Notebook) -> None:
        ...
        self.roll_target_derivative_combo.bind("<<ComboboxSelected>>", self._on_roll_target_selected)
    ...
    def _schedule_market_panel_refresh(self, *, initial_delay_ms: int | None = None) -> None:
        ...
        due_at = time.monotonic() + delay / 1000
        if self._market_panel_job is not None and self._market_panel_job_due_at is not None:
            if self._market_panel_job_due_at <= due_at:
                return
            try:
                self.window.after_cancel(self._market_panel_job)
            except Exception:
        ...
        self._market_panel_job_due_at = due_at
    ...
    def _wait_market_panel_public_ws_update(
        self,
        *,
        refresh_token: int,
        environment: str,
        inst_ids: tuple[str, str],
        after_version: int,
        timeout_ms: int,
    ) -> None:
        self._market_panel_wait_token += 1
        wait_token = self._market_panel_wait_token

        def _worker() -> None:
            timeout_seconds = max(timeout_ms / 1000, 0.1)
            try:
                self.client.wait_public_market_update(
                    inst_ids,
            ...
            def _apply() -> None:
                ...
                if refresh_token != self._market_panel_refresh_token:
                    return
                if wait_token != self._market_panel_wait_token:
                    return
                self._run_market_panel_refresh()
        ...
        threading.Thread(target=_worker, name="market-panel-ws-wait", daemon=True).start()
    ...
    def _fetch_market_panel_snapshot_worker(self, refresh_token: int, context) -> None:
        ...
        ws_after_version: int | None = None
        try:
            ...
            if using_public_ws:
                ...
                spot_ticker_version, spot_ticker = spot_ticker_payload
                derivative_ticker_version, derivative_ticker = derivative_ticker_payload
                spot_order_book_version, spot_order_book = spot_order_book_payload
                derivative_order_book_version, derivative_order_book = derivative_order_book_payload
                ws_after_version = max(
                    spot_ticker_version,
                    derivative_ticker_version,
                    spot_order_book_version,
                    derivative_order_book_version,
                )
                source_mode = "WS"
        ...
        def _apply() -> None:
            ...
            if (
                self._fast_mode
                and ws_after_version is not None
                and ws_after_version > 0
            ):
                self._wait_market_panel_public_ws_update(
                    refresh_token=refresh_token,
                    environment=environment,
                    inst_ids=(spot_inst_id, derivative_inst_id),
                    after_version=ws_after_version,
                    timeout_ms=self._market_panel_refresh_interval_ms,
                )
                return
            self._schedule_market_panel_refresh()
    ...
    def _configure_manager_callbacks(self) -> None:
        set_open_callback = getattr(self.manager.auto_open, "set_status_callback", None)
        if callable(set_open_callback):
            set_open_callback(self._handle_auto_monitor_session_update)
        set_close_callback = getattr(self.manager.auto_close, "set_status_callback", None)
        if callable(set_close_callback):
            set_close_callback(self._handle_auto_monitor_session_update)
        if self._widget_exists(getattr(self, "window", None)):
            self._schedule_monitor_refresh()

    def _handle_auto_monitor_session_update(self, _session=None) -> None:
        try:
            self._safe_after(self._refresh_monitor_status_from_sessions)
        except Exception:
            pass

    def _refresh_monitor_status_from_sessions(self) -> None:
        window = getattr(self, "window", None)
        if getattr(self, "_destroying", False) or not self._widget_exists(window):
            return
        ...
        session_kind = "open"
        if session is None:
            session = self.manager.auto_close.session
            session_kind = "close"
        if session is None:
            self.monitor_status_text.set("未监控")
            return
        parts = [session.status]
        if getattr(session, "last_spread_pct", None) is not None:
            parts.append(f"价差率 {format_decimal_fixed(getattr(session, 'last_spread_pct'), 4)}%")
        ...
        if result is None:
            return
        result_key = (session_kind, id(result))
        if self._last_auto_monitor_result_key == result_key:
            return
        self._last_auto_monitor_result_key = result_key
        self._reload_ledger()

    def _schedule_trade_post_execution_followups(
        self,
        *,
        success: bool,
        message: str,
        ...
        def _reload_ledger_if_needed() -> None:
            if self._destroying or not self._widget_exists(self.window) or not reload_ledger:
                return
            self._reload_ledger()

        def _show_dialog_if_needed() -> None:
            if self._destroying or not self._widget_exists(self.window):
                return
            if success and self._fast_mode:
                return
            if success:
                messagebox.showinfo(success_title, message, parent=self.window)
            else:
                messagebox.showerror(error_title, message, parent=self.window)

        if not self._fast_mode:
            _reload_ledger_if_needed()
            _show_dialog_if_needed()
            return

        if reload_ledger:
            self._safe_after(_reload_ledger_if_needed, delay_ms=80)
        self._safe_after(_show_dialog_if_needed, delay_ms=120 if not success else 650)

    def _get_live_ticker_with_version(self, inst_id: str, *, environment: str) -> tuple[int | None, OkxTicker]:
        try:
            self.client.ensure_public_ws_market_watch(inst_id, environment=environment)
        except Exception:
            pass
        try:
            payload = self.client.get_cached_public_ticker(inst_id, environment=environment)
        except Exception:
            payload = None
        if payload is not None:
            version, ticker = payload
            return version, ticker
        return None, self.client.get_ticker(inst_id)
    ...
    def _wait_public_market_update_or_sleep(
        ...
        *,
        ...
        timeout_seconds: float = PAIR_CLOSE_MONITOR_POLL_SECONDS,
    ) -> None:
        ...
        if after_version is not None and after_version > 0:
            while not stop_event.is_set():
                ...
                if remaining <= 0:
                    return
                wait_seconds = min(remaining, PUBLIC_WS_MONITOR_WAIT_SLICE_SECONDS)
                try:
                    version = self.client.wait_public_market_update(
                        inst_ids,
                        environment=environment,
                        after_version=after_version,
    ...
    def _schedule_roll_post_execution_followups(
        self,
        *,
        success: bool,
        message: str,
        ...
        def _refresh_positions_if_needed() -> None:
            if self._destroying or not self._widget_exists(self.window) or not refresh_positions:
                return
            self._refresh_roll_positions()
        ...
        if refresh_positions:
            self._safe_after(_refresh_positions_if_needed, delay_ms=300 if success else 80)
        self._safe_after(_show_dialog_if_needed, delay_ms=120 if not success else 650)
    ...
    def _rebuild_manager_for_selected_api(self) -> None:
        ...
        self._configure_manager_callbacks()
    ...
    def _selected_ledger_entry(self):
        ledger_tree = getattr(self, "ledger_tree", None)
    ...
    def _refresh_close_entry_options(self) -> None:
        ...
        close_entry_combo = getattr(self, "close_entry_combo", None)
        if close_entry_combo is None:
            return
        close_entry_combo.configure(values=labels)
        if not labels:
    ...
    def _selected_roll_entry(self):
        ...
        spot_position = self._roll_spot_by_base.get(base_ccy)
        spot_instrument = self._roll_instruments.get(spot_position.inst_id) if isinstance(spot_position, OkxPosition) else None
        source_entry = None
        if self._roll_source_entry_id:
            entry = self._ledger_entry_by_id.get(self._roll_source_entry_id)
            if (
                entry is not None
                and getattr(entry, "close_mode", "") == "open"
                and entry.derivative_inst_id == derivative_position.inst_id
                and entry.base_ccy == base_ccy
            ):
                source_entry = entry
        ...
        if derivative_qty <= 0:
            return None
        if spot_position is not None and spot_instrument is not None:
            spot_inst_id = spot_position.inst_id
            ...
            spot_inst_id = getattr(source_entry, "spot_inst_id", "") or f"{base_ccy}-USDT"
            if source_entry is not None:
                raw_spot_qty = source_entry.spot_qty
                if derivative_qty < source_entry.derivative_qty:
                    raw_spot_qty = source_entry.spot_qty * derivative_qty / max(source_entry.derivative_qty, Decimal("1e-18"))
            else:
                raw_spot_qty = (
            ...
            try:
                spot_inst = self.manager.get_instrument(spot_inst_id)
            except Exception:
                spot_inst = None
            if spot_inst is not None:
                spot_qty = snap_to_increment(max(raw_spot_qty, Decimal("0")), spot_inst.lot_size, "down")
            else:
                spot_qty = max(raw_spot_qty, Decimal("0"))
    ...
    def _refresh_roll_positions(self) -> None:
        ...
        if runtime is None:
            return
        if self._roll_positions_refresh_busy:
            self._roll_positions_refresh_requested = True
            return
        self._roll_positions_refresh_busy = True
        self._roll_positions_refresh_requested = False
        ...
        def _worker() -> None:
            ...
            def _run_with_timeout(task_name: str, timeout_seconds: float, callback: Callable[[], object]) -> object:
                ...
                thread = threading.Thread(
                    target=_target,
                    daemon=True,
                    name=f"roll-refresh-{task_name}",
                )
                thread.start()
                thread.join(timeout=max(timeout_seconds, 0.1))
                if thread.is_alive():
                    raise TimeoutError(f"{task_name} 超时 {timeout_seconds:.1f}s")
                if error_holder:
                    raise error_holder[0]
                return result_holder[0] if result_holder else None

            def _load_futures_positions() -> list[OkxPosition]:
                if not self._fast_mode:
                    return list(
                ...
                if cached:
                    return cached
                status_note_parts.append("缓存空，已REST复核")
                return list(
                    self.client.get_positions(
                        runtime.credentials,
                        environment=runtime.environment,
            ...
            try:
                status_note_parts: list[str] = []
                if self._fast_mode:
                    derivative_positions = list(_load_futures_positions())
                else:
                    derivative_positions = list(
                        _run_with_timeout(
                            "futures-positions",
                            ROLL_POSITION_FETCH_TIMEOUT_SECONDS,
                            _load_futures_positions,
                        )
                    )
                if status_note_parts:
                    status_note = " | " + "；".join(status_note_parts)
                spot_positions: list[OkxPosition] = []
                if not self._fast_mode:
                    try:
                        overview = _run_with_timeout(
                            "account-overview",
                            ROLL_OVERVIEW_FETCH_TIMEOUT_SECONDS,
                            lambda: self.client.get_account_overview(
                                runtime.credentials,
                                environment=runtime.environment,
                                prefer_cache=False,
                            ),
                        )
                        spot_positions = _build_spot_positions_from_account(
                            overview,
                            self.client,
                            validate_instrument=False,
                        )
                    except Exception as exc:  # noqa: BLE001
                        status_note = f" | 现货补充慢：{exc}"
                positions = spot_positions + derivative_positions
                for position in positions:
                    instrument = self.client.get_cached_instrument(position.inst_id)
                    if instrument is None:
                        try:
                            instrument = self.client.get_instrument(position.inst_id, prefer_cached=True)
                        except Exception:
                            instrument = None
            ...
            def _apply() -> None:
                self._roll_positions_refresh_busy = False
                rerun = self._roll_positions_refresh_requested
                self._roll_positions_refresh_requested = False
                if error is not None:
                    self.roll_status_text.set(f"当前持仓读取失败：{error}")
                    self._append_log(f"交割移仓持仓刷新失败：{error}")
                    if rerun:
                        self._safe_after(self._refresh_roll_positions, delay_ms=50)
                    return
                ...
                spot_count = sum(1 for item in positions if item.inst_type == "SPOT")
                futures_count = sum(1 for item in positions if item.inst_type == "FUTURES")
                candidate_count = len(self._roll_position_by_key)
                if self._fast_mode:
                    self.roll_status_text.set(
                        f"已读取交割持仓 {futures_count} 条 | 候选 {candidate_count}{status_note}"
                    )
                else:
                    self.roll_status_text.set(
                        f"已读取当前持仓 {len(positions)} 条 | 现货 {spot_count} | 交割 {futures_count} | 候选 {candidate_count}{status_note}"
                    )
                self._append_log(
                    f"交割移仓持仓已刷新：总计 {len(positions)} 条，现货 {spot_count}，交割 {futures_count}，候选 {candidate_count}{status_note}"
                )
                self._schedule_market_panel_refresh(initial_delay_ms=50)
                if rerun:
                    self._safe_after(self._refresh_roll_positions, delay_ms=50)
    ...
    def _refresh_roll_entry_options(self) -> None:
        ...
        if not derivative_values:
            self._roll_entry_label.set("")
            self.roll_contract_qty.set("")
            self.roll_position_summary_text.set("当前没有可用于移仓的交割合约持仓。")
            self.roll_preview_text.set("请先刷新当前持仓，或确认当前 API 账户里存在可回补的交割合约仓位。")
            if self._roll_market_panel is not None:
                self._clear_market_panel(self._roll_market_panel, message="当前没有可用于移仓的交割合约持仓。")
            return
        target_label = next(
    ...
    def _on_roll_entry_selected(self, _event=None) -> None:
        ...
        if entry is None or not getattr(entry, "entry_id", ""):
            self._roll_source_entry_id = None
        if not self.roll_contract_qty.get().strip() and entry is not None:
            self.roll_contract_qty.set(format_decimal(entry.derivative_qty))
        self._refresh_roll_target_options()
        self._refresh_roll_preview()
        self._schedule_market_panel_refresh(initial_delay_ms=50)

    def _on_roll_target_selected(self, _event=None) -> None:
        target_derivative_inst_id = self.roll_target_derivative_inst_id.get().strip().upper()
        if target_derivative_inst_id:
            self.roll_status_text.set(f"正在切换目标交割合约：{target_derivative_inst_id}")
        self._schedule_market_panel_refresh(initial_delay_ms=50)
        self._refresh_roll_preview()
    ...
    def _refresh_roll_preview(self) -> None:
        ...
        try:
            ...
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return
        self._roll_preview_refresh_token += 1
        preview_token = self._roll_preview_refresh_token
        environment = self._selected_api_environment()
        self.roll_preview_text.set(f"正在刷新 {target_derivative_inst_id} 的移仓预览...")
        self._schedule_market_panel_refresh(initial_delay_ms=50)
        ...
        def _worker() -> None:
            ...
            try:
                ...
                _, current_ticker = self._get_live_ticker_with_version(
                    entry.derivative_inst_id,
                    environment=environment,
                )
                _, target_ticker = self._get_live_ticker_with_version(
                    target_derivative_inst_id,
                    environment=environment,
                )
            ...
            def _apply() -> None:
                if self._destroying or not self._widget_exists(self.window):
                    return
                if preview_token != self._roll_preview_refresh_token:
                    return
                if error is not None:
                    self.roll_preview_text.set(f"预览失败：{error}")
                elif preview_text is not None:
                    self.roll_preview_text.set(preview_text)
                self._schedule_market_panel_refresh(initial_delay_ms=50)
    ...
    def _refresh_pair_close_positions(self) -> None:
        ...
        if runtime is None:
            return
        if self._pair_close_positions_refresh_busy:
            self._pair_close_positions_refresh_requested = True
            return
        self._pair_close_positions_refresh_busy = True
        self._pair_close_positions_refresh_requested = False
        ...
        def _worker() -> None:
            ...
            reference_prices: dict[str, Decimal] = {}
            try:
                derivative_positions = self.client.get_positions(
                    runtime.credentials,
                    environment=runtime.environment,
                    prefer_cache=False,
                )
            ...
            def _apply() -> None:
                self._pair_close_positions_refresh_busy = False
                rerun = self._pair_close_positions_refresh_requested
                self._pair_close_positions_refresh_requested = False
                if error is not None:
                    self.pair_close_status_text.set(f"当前持仓读取失败：{error}")
                    if rerun:
                        self._safe_after(self._refresh_pair_close_positions, delay_ms=50)
                    return
    ...
    def _refresh_roll_live_spread(
        ...
        *,
        ...
        current_version, current_ticker = self._get_live_ticker_with_version(
            current_derivative_inst_id,
            environment=environment,
        )
        target_version, target_ticker = self._get_live_ticker_with_version(
            target_derivative_inst_id,
            environment=environment,
        )
        current_buy = current_ticker.ask or current_ticker.last
        ...
        versions = [version for version in (current_version, target_version) if version is not None]
        return spread_pct, spread_abs, (max(versions) if versions else None)
    ...
    def _refresh_pair_close_live_spread(
        self,
        spot_inst_id: str,
        derivative_inst_id: str,
        *,
        ...
        spot_version, spot_ticker = self._get_live_ticker_with_version(spot_inst_id, environment=environment)
        derivative_version, derivative_ticker = self._get_live_ticker_with_version(
            derivative_inst_id,
            environment=environment,
        )
        ...
        spread_abs = derivative_mid - spot_mid
        spread_pct = spread_abs / spot_mid * Decimal("100")
        versions = [version for version in (spot_version, derivative_version) if version is not None]
        return spread_pct, spread_abs, (max(versions) if versions else None)
        ...
        def _worker() -> None:
            ...
            try:
                while not self._pair_close_auto_stop_event.is_set():
                    spread_pct, spread_abs, market_version = self._refresh_pair_close_live_spread(
                        session.spot_inst_id,
                        session.derivative_inst_id,
                        environment=runtime.environment,
                    )
                    ...
                    self._wait_public_market_update_or_sleep(
                        environment=runtime.environment,
                        inst_ids=(session.spot_inst_id, session.derivative_inst_id),
                        after_version=market_version,
                        stop_event=self._pair_close_auto_stop_event,
                    )
        ...
        def _worker() -> None:
            ...
            try:
                while not self._roll_auto_stop_event.is_set():
                    spread_pct, spread_abs, market_version = self._refresh_roll_live_spread(
                    ...
                    self._wait_public_market_update_or_sleep(
                        environment=runtime.environment,
                        inst_ids=(session.current_derivative_inst_id, session.target_derivative_inst_id),
                        after_version=market_version,
                        stop_event=self._roll_auto_stop_event,
                    )
            ...
            def _apply() -> None:
                ...
                if error is None and session.triggered:
                    self._schedule_roll_post_execution_followups(
                        success=True,
                        message=message,
                        success_title="自动移仓完成",
                        error_title="自动移仓失败",
                        reload_ledger=True,
    ...
    def _schedule_monitor_refresh(self) -> None:
        self._refresh_monitor_status_from_sessions()
    ...
    def _start_auto_open(self) -> None:
        ...
        try:
            ...
            return
        self._last_auto_monitor_result_key = None
    ...
    def _open_now(self) -> None:
        ...
        def _worker() -> None:
            ...
            def _apply() -> None:
                ...
                self._append_log(result.message)
                self._schedule_trade_post_execution_followups(
                    success=result.success,
                    message=result.message,
                    success_title="开仓完成",
                    error_title="开仓失败",
                    reload_ledger=result.success,
                )
    ...
    def _invalidate_roll_market_refresh(self) -> None:
        self._market_panel_refresh_token += 1
        self._market_panel_wait_token += 1
        self._roll_preview_refresh_token += 1
    ...
    def _on_roll_target_selected(self, _event=None) -> None:
        target_derivative_inst_id = self.roll_target_derivative_inst_id.get().strip().upper()
        self._invalidate_roll_market_refresh()
        if target_derivative_inst_id:
            self.roll_status_text.set(f"正在切换目标交割合约：{target_derivative_inst_id}")
            if self._roll_market_panel is not None:
                self._clear_market_panel(self._roll_market_panel, message=f"正在切换盘口：{target_derivative_inst_id}")
        self._schedule_market_panel_refresh(initial_delay_ms=50)
        self._refresh_roll_preview()
    ...
    def _refresh_roll_preview(self) -> None:
        derivative_position = self._selected_roll_position()
        entry = self._selected_roll_entry()
        if derivative_position is None or entry is None:
            self.roll_position_summary_text.set("请先刷新并选择一条当前交割合约持仓。")
            self.roll_preview_text.set("请选择当前交割合约持仓，并填写更远交割合约。")
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return
        ...
        if derivative_instrument is None:
            try:
                derivative_instrument = self.manager.get_instrument(derivative_position.inst_id)
                self._roll_instruments[derivative_position.inst_id] = derivative_instrument
            except Exception:
                derivative_instrument = None
        if derivative_instrument is None:
            self.roll_preview_text.set("当前缺少交割合约元数据，请先刷新当前持仓。")
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return

        source_entry = self._ledger_entry_by_id.get(entry.entry_id) if getattr(entry, "entry_id", "") else None
        spot_position = self._roll_spot_by_base.get(entry.base_ccy)
        spot_instrument = self._roll_instruments.get(spot_position.inst_id) if isinstance(spot_position, OkxPosition) else None
        spot_line = (
            f"现货持仓：{_pair_position_label(spot_position, spot_instrument)}"
            if isinstance(spot_position, OkxPosition)
            else f"现货持仓：未读取到实时现货，按 {entry.spot_inst_id} 估算 {format_decimal(entry.spot_qty)} {entry.base_ccy}"
        )
        self.roll_position_summary_text.set(
            "\n".join(
                [
                    spot_line,
                    f"当前交割：{_pair_position_label(derivative_position, derivative_instrument)}",
                    ("账本关联：已关联当前账本选中记录" if source_entry is not None else "账本关联：未绑定，按现有交割合约持仓直接移仓"),
                ]
            )
        )
        ...
        if not target_derivative_inst_id:
            ...
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return
        if target_derivative_inst_id == entry.derivative_inst_id:
            self.roll_preview_text.set("目标交割合约不能和当前交割合约相同。")
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return
        if entry.base_ccy != target_derivative_inst_id.split("-")[0].strip().upper():
            self.roll_preview_text.set("目标交割合约的币种必须和当前持仓一致。")
            self._schedule_market_panel_refresh(initial_delay_ms=50)
            return
        ...
        base_preview_lines = [
            f"本次移仓：回补 {entry.derivative_inst_id} {format_decimal(derivative_qty)} 张",
            f"并开出 {target_derivative_inst_id} {format_decimal(derivative_qty)} 张",
            f"对应继续占用现货：{format_decimal(entry.spot_qty)} {entry.base_ccy}",
            f"分批计划：{_format_batch_plan_summary(planned_batches)}",
            f"执行方式：{self.roll_execution_mode_label.get().strip()} | 最大滑点：{self.max_slippage_percent.get().strip()}%",
        ]
        self.roll_preview_text.set("\n".join([*base_preview_lines, "盘口价差：正在等待 WS/后台行情刷新..."]))
        self._schedule_market_panel_refresh(initial_delay_ms=50)
        ...
        def _worker() -> None:
            ...
            def _apply() -> None:
                ...
                if preview_token != self._roll_preview_refresh_token:
                    return
                if error is not None:
                    self.roll_preview_text.set("\n".join([*base_preview_lines, f"盘口价差：后台刷新失败：{error}"]))
                else:
                    self.roll_preview_text.set(
                        "\n".join([*base_preview_lines, f"当前移仓绝对价差：{format_decimal(spread_abs)}"])
                    )
                self._schedule_market_panel_refresh(initial_delay_ms=50)
    ...
    def _build_roll_request(self, *, entry, roll_derivative_qty: Decimal) -> ArbitrageRollRequest:
        ...
        return ArbitrageRollRequest(
            ...
            chase_limit=self._parse_roll_chase_limit(),
            base_ccy=entry.base_ccy,
            spot_inst_id=entry.spot_inst_id or f"{entry.base_ccy}-USDT",
    ...
    def _reload_ledger(self) -> None:
        ...
        if not entries:
            if ledger_tree is not None:
                ledger_tree.insert("", END, iid="placeholder", values=("—", "暂无套利账本记录", "", "", "", "", "", "", "", ""))

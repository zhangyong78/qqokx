...
VARIANTS = (
    ReentryVariant(
        key="baseline",
        label="原策略",
        note="平仓后只要斜率条件仍满足，就允许下一次开空，不做额外状态管理。",
    ),
    ReentryVariant(
        key="locked_only_ema21_near",
        label="仅锁盈后EMA21接近再开",
        note="只针对 locked_xr_stop：先反抽到 EMA21 附近，再重新跌回 EMA21 下方，才允许再空。",
        locked_rule="near",
    ),
    ReentryVariant(
        key="protective_ema21_near",
        label="保护性出场EMA21接近再开",
        note="break_even_stop 和 locked_xr_stop 都要先反抽到 EMA21 附近，再跌回 EMA21 下方。",
        break_even_rule="near",
        locked_rule="near",
    ),
    ReentryVariant(
        key="protective_ema21_reclaim",
        label="保护性出场上穿EMA21再开",
        note="break_even_stop 和 locked_xr_stop 都要先站回 EMA21 上方，再重新跌回 EMA21 下方。",
        break_even_rule="reclaim",
        locked_rule="reclaim",
    ),
    ReentryVariant(
        key="reasoned_split",
        label="分原因状态机",
        note="stop_loss 冷却 6 根；break_even_stop 冷却 3 根；locked_xr_stop 必须先接近 EMA21 再跌回 EMA21 下方。",
        stop_loss_cooldown_bars=6,
        break_even_rule="cooldown",
...
def main() -> None:
    ...
    basename = f"ema55_slope_short_reentry_state_machine_5coins_10u_{stamp}"
...
def simulate_trades(df: pd.DataFrame, variant: ReentryVariant) -> pd.DataFrame:
    trades: list[dict[str, object]] = []
    position: dict[str, float | int | str] | None = None
    blocked_until_index = -1
    reset_state: str | None = None
    reset_rule: str | None = None
    reset_line_name = "ema21"
    ...
    for index in range(start_index, len(df)):
        ...
        if position is not None:
            ...
            if closed_trade is not None:
                ...
                position = None
                gate_type = handle_exit_gate(variant, exit_reason, index)
                if gate_type == "stop_loss_cooldown":
                    blocked_until_index = max(blocked_until_index, index + variant.stop_loss_cooldown_bars)
                elif gate_type == "break_even_cooldown":
                    blocked_until_index = max(blocked_until_index, index + variant.break_even_cooldown_bars)
                elif gate_type in {"break_even_near", "break_even_reclaim", "locked_near", "locked_reclaim"}:
                    reset_state = "await_rebound"
                    reset_rule = gate_type.rsplit("_", 1)[-1]
        ...
        if reset_state is not None:
            ...
            if not reset_ready(reset_rule, reset_state, close_price, line_value, atr_value):
                continue
        ...
        position = {
            ...
            "reentry_gate_type": reset_rule or "",
        }
        gate_trigger_count = 0
...
def handle_exit_gate(variant: ReentryVariant, exit_reason: str, index: int) -> str | None:
    if exit_reason == "stop_loss" and variant.stop_loss_cooldown_bars > 0:
        return "stop_loss_cooldown"
    if exit_reason == "break_even_stop":
        if variant.break_even_rule == "cooldown" and variant.break_even_cooldown_bars > 0:
            return "break_even_cooldown"
        if variant.break_even_rule == "near":
            return "break_even_near"
        if variant.break_even_rule == "reclaim":
            return "break_even_reclaim"
        return None
    if is_locked_exit(exit_reason):
        if variant.locked_rule == "near":
            return "locked_near"
        if variant.locked_rule == "reclaim":
            return "locked_reclaim"
    return None


def reset_ready(rule: str | None, state: str, close_price: float, line_value: float, atr_value: float) -> bool:
    if rule == "near":
...
def flatten_coin_metrics(*, symbol: str, variant: ReentryVariant, trades: pd.DataFrame) -> dict[str, object]:
...
<head>
  ...
  <title>EMA55 斜率做空再入场状态机报告</title>
  <style>
    :root {{
...
<body>
  <div class="wrap">
    <section class="hero">
      <div class="eyebrow">Slope Short / Re-entry State Machine</div>
      <h1>EMA55 斜率做空再入场状态机</h1>
      <p>这轮只回答一个问题：平仓后是否应该立刻重开。我们保持原入场、原出场、原风险金完全不变，只替换“下一次什么时候允许再空”的状态机。</p>
    </section>
    <section class="grid">
      ...
      <div class="card wide"><h2>测试口径</h2><ul>
        ...
        <li>出场：原策略动态止盈，2R 保本，之后按 locked-R 逐级锁盈</li>
        <li>只改再入场状态机，不混入新的止盈规则，不混入新的入场过滤</li>
      </ul></div>
      <div class="card wide"><h2>状态机版本</h2><div class="mode-grid">{mode_cards}</div></div>
      ...
      <div class="card full"><h2>模式总表</h2><div class="note">`gate_trade_rate` 表示有多少交易是在某次再入场门槛生效后才重新进入，用来判断规则是不是只停留在纸面上。</div><div class="scroll" style="margin-top:12px;">{summary_table}</div></div>
      ...
      <div class="card wide"><h2>各币种门槛触发率</h2><img src="data:image/png;base64,{render_coin_heatmap(coin_frame, "test_gate_trade_rate", "各币种测试段 gate_trade_rate")}" alt="coin_gate"></div>

...
ATR_TAKE_RATIOS = (Decimal("1"), Decimal("2"), Decimal("3"))
CANDLE_LIMIT = 10000
ATR_PERIOD = 10
            ...
            for ema_period, trend_ema_period in EMA_PROFILES:
                ...
                for entry_reference_ema_period in ENTRY_REFERENCE_EMAS:
                    ...
                    for stop_atr in ATR_STOP_MULTIPLIERS:
                        for take_ratio in ATR_TAKE_RATIOS:
        ...
        for row in rows:
            writer.writerow(
                [
                    ...
                    format_decimal(row.atr_take_multiplier),
...
def build_text_report(rows: list[SweepRow], data_source_note: str, exported_at: datetime, csv_path: Path) -> str:
    ...
    lines = [
        ...
        "止盈止损：SL = 1 / 1.5 / 2 ATR；TP = 1 / 2 / 3 倍SL",
    ...
    for key in sorted(best_by_group):
        ...
        lines.append(
            ...
            f"SLx{format_decimal(row.atr_stop_multiplier)} / TPx{format_decimal(row.atr_take_multiplier)} | "
            ...
            f"盈亏/回撤 {format_decimal_fixed(row.pnl_dd_ratio, 4)}"
        )
    ...
    lines.extend(
        [
            "",
            "四、交易员分析角度",
            "1. 先看同方向里，快均线组合是否明显优于慢均线组合，判断ETH当前更吃启动段还是趋势延续段。",
            "2. 再看挂单参考EMA是更偏快线还是更偏慢线，判断回调挂单应该贴近价格还是更保守。",
            "3. 把总盈亏和最大回撤一起看，不能只按收益排名；若收益只多一点但回撤明显放大，实盘未必更优。",
            "4. 看交易次数，快均线通常交易更密集；如果收益提升不够覆盖交易频率和成本，慢均线反而更稳。",
            "5. 做多和做空要分开判断，ETH经常会表现出明显的方向不对称。",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
...
def main() -> None:
    ...
    csv_path = REPORTS_DIR / f"ETH_ema_dynamic_ema_sweep_{stamp}.csv"
    txt_path = REPORTS_DIR / f"ETH_ema_dynamic_ema_sweep_{stamp}.txt"
    json_path = REPORTS_DIR / f"ETH_ema_dynamic_ema_sweep_{stamp}.json"
        ...
        json.dumps(
            {
                ...
                "atr_take_ratios": [str(item) for item in ATR_TAKE_RATIOS],
                "rows": len(rows),

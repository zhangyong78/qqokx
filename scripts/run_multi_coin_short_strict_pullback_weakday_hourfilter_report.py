...
HTML_PATH = REPORT_DIR / "multi_coin_short_strict_pullback_weakday_hourfilter_report.html"
CSV_PATH = REPORT_DIR / "multi_coin_short_strict_pullback_weakday_hourfilter_by_coin.csv"
TRADES_CSV_PATH = REPORT_DIR / "multi_coin_short_strict_pullback_weakday_hourfilter_trades.csv"
JSON_PATH = REPORT_DIR / "multi_coin_short_strict_pullback_weakday_hourfilter_summary.json"
...
def main() -> None:
    ...
    blocked_hours_map: dict[str, list[int]] = {}
    ...
    for item in recommendations:
        ...
        blocked_hours = detect_blocked_hours(frame)
        blocked_hours_map[item.coin] = blocked_hours
    ...
    payload = {
        ...
        "assumptions": {
            ...
            "hour_filter": f"按历史1H实体波动率分时统计，屏蔽每个币最躁动的前{BLOCK_TOP_HOURS}个UTC小时",
        },
        "blocked_hours_by_coin": blocked_hours_map,
        "aggregate": {
            "strict_reference_pnl": float(merged["strict_test_pnl_u"].sum()),
            "strict_reference_dd": float(merged["strict_test_max_drawdown_u"].sum()),
            "weakday_hourfilter_pnl": float(merged["test_pnl_u"].sum()),
            "weakday_hourfilter_dd": float(merged["test_max_drawdown_u"].sum()),
        },
    }
    JSON_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    comparison_chart = render_comparison_chart(merged)
...
def build_daily_info(entry_candles: list[object], daily_candles: list[object], filter_key: str) -> list[dict[str, object]]:
    ...
    for candle in entry_candles:
        ...
        if idx < 0:
            out.append({"bias": "neutral", "weak_day": False})
            continue
        ...
        if not np.isfinite(line_value):
            out.append({"bias": "neutral", "weak_day": False})
            continue
        bias = "short" if day_close < line_value else "long" if day_close > line_value else "neutral"
        weak_day = day_close < day_open and day_close < line_value
        out.append({"bias": bias, "weak_day": weak_day})
    return out


def detect_blocked_hours(frame: pd.DataFrame) -> list[int]:
    tmp = frame.copy()
    ...
    return [int(hour) for hour in stat.head(BLOCK_TOP_HOURS).index.tolist()]
...
def render_comparison_chart(merged: pd.DataFrame) -> str:
    labels = ["严格反抽版", "弱势日+时段过滤版"]
    ...
    ax.set_title("严格反抽版 vs 弱势日+时段过滤版")
    ax.legend()
    fig.tight_layout()
...
def render_coin_chart(merged: pd.DataFrame) -> str:
    ...
    width = 0.36
    ax.bar(x - width / 2, merged["strict_test_pnl_u"].astype(float), width, label="严格反抽版", color="#6A9C89")
    ax.bar(x + width / 2, merged["test_pnl_u"].astype(float), width, label="弱势日+时段过滤版", color="#355F2E")
...
def render_equity_chart(trades_frame: pd.DataFrame) -> str:
    ...
    axes[0].set_title("弱势日+时段过滤版累计收益")
    axes[1].fill_between(np.arange(len(ordered)), ordered["dd"].to_numpy(), color="#C84B31", alpha=0.28)
    axes[1].set_title("弱势日+时段过滤版回撤")
    axes[1].set_xlabel("Trade Sequence")
    fig.tight_layout()
    return figure_to_base64(fig)


def build_html(*, merged: pd.DataFrame, payload: dict[str, object], comparison_chart: str, coin_chart: str, equity_chart: str) -> str:
...
<head>
  ...
  <title>反抽50% + 日线弱势日 + 高波动时段过滤</title>
  <style>
...
<body>
  <div class="wrap">
    <section class="hero">
      <div class="eyebrow">10U 战神 / Weak Day Filter</div>
      <h1>反抽不收回 50% + 只做日线弱势日 + 限制高波动时段</h1>
      <p>这版是在“严格反抽版”基础上继续收紧：只做日线明显偏弱的日子，同时避开每个币历史上最躁动的若干 UTC 小时。目标不是多打单，而是提高空头确认质量。</p>
    </section>
    <section class="grid">
      <div class="card stat"><div class="muted">严格反抽版</div><div class="value">{agg["strict_reference_pnl"]:.1f}U</div><div class="muted">测试总收益</div></div>
      <div class="card stat"><div class="muted">弱势日+时段版</div><div class="value">{agg["weakday_hourfilter_pnl"]:.1f}U</div><div class="muted">测试总收益</div></div>
      <div class="card stat"><div class="muted">严格反抽版</div><div class="value">{agg["strict_reference_dd"]:.1f}U</div><div class="muted">测试总回撤</div></div>
      <div class="card stat"><div class="muted">弱势日+时段版</div><div class="value">{agg["weakday_hourfilter_dd"]:.1f}U</div><div class="muted">测试总回撤</div></div>

      <div class="card wide">
        <h2>这版的真实结果</h2>
        <ul>
          <li>如果你要的是“更干净的确认”，它确实更严格了。</li>
          <li>如果你要的是“更赚钱”，这版不一定比严格反抽版更好，关键要看是否把亏损币修掉的同时没有把赚钱机会砍太多。</li>
          <li>这里最重要的不是总收益一个数字，而是 ETH、SOL 这些弱项有没有改善。</li>
        </ul>
      </div>
      <div class="card wide">
        <h2>本次假设</h2>
      ...
      <div class="card wide"><h2>总览图</h2><img src="data:image/png;base64,{comparison_chart}" alt="comparison"></div>
      <div class="card wide"><h2>各币收益图</h2><img src="data:image/png;base64,{coin_chart}" alt="coin chart"></div>
      <div class="card full"><h2>资金曲线</h2><img src="data:image/png;base64,{equity_chart}" alt="equity"></div>
      <div class="card full"><h2>被屏蔽的高波动 UTC 小时</h2><ul>{blocked_list}</ul></div>

...
from scripts.build_best_parameter_bundle import SHORT_SPECS, build_slope_short_config
...
BASENAME = f"best_short_btc_eth_sol_monthly_yearly_compare_{STAMP}"
...
PROJECT_HTML_PATH = PROJECT_REPORT_DIR / "best_short_btc_eth_sol_monthly_yearly_compare.html"
...
def build_config_label(spec) -> str:
    line = f"{spec.ema_type.upper()}{spec.ema_period}"
    if spec.strategy_id == "body_retest_short":
        return f"Body/ATR 回抽做空 | 基准线 {line}"
    return f"{line} 斜率做空"


def build_daily_filter_label(spec) -> str:
    if spec.daily_filter_mode == "weak_day":
        return "北京时间8点日线 Weak Day"
    return f"北京时间8点日线 {spec.daily_filter_ma_type.upper()}{spec.daily_filter_period}"


def run_study(client: OkxRestClient, spec) -> Study:
    ...
    return Study(
        ...
        config_label=build_config_label(spec),
        daily_filter_label=build_daily_filter_label(spec),
        trades=trades,
...
def build_yearly_chart(yearly_pivot: pd.DataFrame) -> str:
    ...
    ax.set_title("BTC / ETH / SOL 做空最佳参数年度利润横向对比")
    ax.set_ylabel("PnL (U)")
...
def build_monthly_total_chart(monthly_pivot: pd.DataFrame) -> str:
    ...
    ax.set_title("BTC / ETH / SOL 做空最佳参数月度总利润")
    ax.set_ylabel("PnL (U)")
    ax.grid(axis="y", alpha=0.2)
    return fig_to_base64(fig)
        ...
        [
            {
                ...
                "risk_model": "10U 固定风险 / ATR14 止损2 / 动态止盈 / 2R保本 / ATR分位<=0.5",
            }
            for study in studies
        ]
    )
...
<head>
  <meta charset="utf-8" />
  <title>BTC ETH SOL 做空最佳参数月度年度对比</title>
  <style>
    :root {{
      --bg: #f5f1e8;
      --panel: #fffdf8;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .two-col {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(420px, 1fr)); gap: 16px; }}
...
<body>
  <div class="wrap">
    <section class="hero">
      <h1>BTC / ETH / SOL 做空最佳参数报告</h1>
      <p>口径：只统计当前“最佳参数组合包”里的三条做空定稿配置，统一使用 <strong>1H</strong>、<strong>10U 固定风险</strong>、<strong>北京时间 8 点日线过滤</strong>、<strong>动态止盈 + 2R 保本</strong> 的正式回测接口全历史重跑。</p>
      <div class="grid">
        <div class="card"><div class="label">三币全样本利润</div><div class="value">{format_u(total_all_pnl)}</div></div>
        <div class="card"><div class="label">三币测试段利润</div><div class="value">{format_u(total_test_pnl)}</div></div>
        <div class="card"><div class="label">三币全样本交易数</div><div class="value">{total_trades}</div></div>
    ...
    <section class="section">
      <h2>参数定稿</h2>
      <div class="note">
        这次只看 `BTC / ETH / SOL` 三条做空正式参数，不混入 `BNB Body/ATR`，也不混入 `DOGE`。重点是看这三条定稿空头在不同年份、不同月份的横向贡献。
      </div>
...
def main() -> None:
    ...
    studies = [run_study(client, spec) for spec in selected_short_specs()]

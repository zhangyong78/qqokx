...
SLOPE_THRESHOLDS = (-0.0005, -0.0006, -0.0007, -0.0008, -0.0009, -0.0010)
STOP_ATR_MULTIPLIERS = (1.0, 1.5, 2.0)

CSV_PATH = REPORT_DIR / "r001_slope_atr_matrix.csv"
JSON_PATH = REPORT_DIR / "r001_slope_atr_matrix_summary.json"
HTML_PATH = REPORT_DIR / "r001_slope_atr_matrix_report.html"
HEATMAP_ALL_AVG_R = REPORT_DIR / "r001_slope_atr_matrix_all_avg_r.png"
HEATMAP_TEST_AVG_R = REPORT_DIR / "r001_slope_atr_matrix_test_avg_r.png"
HEATMAP_TEST_PF = REPORT_DIR / "r001_slope_atr_matrix_test_pf.png"
HEATMAP_TEST_TRADES = REPORT_DIR / "r001_slope_atr_matrix_test_trades.png"
...
def main() -> None:
    ...
    save_heatmap(
        ...
        metric="test_avg_r",
        title="测试段 Avg R 热力图",
        output_path=HEATMAP_TEST_AVG_R,
        fmt=".3f",
    )
    save_heatmap(
        comparison,
        metric="test_profit_factor",
        title="测试段 Profit Factor 热力图",
        output_path=HEATMAP_TEST_PF,
        fmt=".2f",
    )
    save_heatmap(
        comparison,
        metric="test_trades",
        title="测试段交易数热力图",
        output_path=HEATMAP_TEST_TRADES,
        fmt=".0f",
    )
...
def score_row(row: dict[str, object]) -> float:
    return (
        ...
        + float(row["validation_avg_r"]) * 110.0
        ...
        - float(row["all_max_drawdown_r"]) * 1.2
    )
...
def save_heatmap(
    ...
    *,
    ...
    pivot = (
        comparison.pivot(index="slope_threshold", columns="stop_atr_mult", values=metric)
...
def build_html(
    *,
    ...
    stable_rows = comparison[
        (comparison["validation_avg_r"] > 0) & (comparison["test_avg_r"] > 0) & (comparison["test_profit_factor"] > 1.0)
    ].copy()
    best_validation_avg_r = float(comparison["validation_avg_r"].max()) if not comparison.empty else 0.0
    strongest_stop = (
        comparison.groupby("stop_atr_mult")["test_avg_r"].mean().sort_values(ascending=False).index[0]
        if not comparison.empty
        else STOP_ATR_MULTIPLIERS[0]
    )
    strongest_slope = (
        comparison.groupby("slope_threshold")["test_avg_r"].mean().sort_values(ascending=False).index[0]
        if not comparison.empty
        else SLOPE_THRESHOLDS[0]
    )
    stability_text = (
        f"当前共有 <strong>{len(stable_rows)}</strong> 组同时满足验证段为正、测试段为正且测试 PF &gt; 1。"
        if len(stable_rows) > 0
        else "当前 <span class=\"bad\">没有任何组合</span> 同时满足验证段为正、测试段为正且测试 PF &gt; 1，这说明这轮参数延展还没有给出足够稳的样本外证据。"
    )
    validation_text = (
        f"所有组合里最好的验证段 Avg R 也只有 <strong>{best_validation_avg_r:.3f}</strong>。"
        if best_validation_avg_r > 0
        else f"所有组合的验证段 Avg R 都为负，最好的也只有 <strong>{best_validation_avg_r:.3f}</strong>。"
    )
...
<title>R001 斜率 x ATR 矩阵报告</title>
<style>
:root {{
  --bg:#f3f5f8; --panel:#ffffff; --ink:#152032; --muted:#5f6b7a; --line:#d9e0e7;
  --hero-a:#0f172a; --hero-b:#1f3a5f; --good:#0f766e; --warn:#b45309; --bad:#be123c;
}}
...
.hero p {{ margin:6px 0; max-width:1120px; color:#dbe6f3; line-height:1.7; }}
...
<body>
<section class="hero">
  <h1>R001 参数延展: 斜率 x ATR 矩阵测试</h1>
  <p>本次按照现有 <strong>EMA55 slope short</strong> 的基线回测逻辑扩展参数矩阵。仓库里没有找到单独落盘的 R001 参数文件，所以这里把它解释为现有这条 slope 短空研究线的基准口径，再对 <strong>斜率阈值 -0.0005 到 -0.0010</strong> 做 6 组延展，并和 <strong>ATR 1 / 1.5 / 2</strong> 组合成 18 组测试。</p>
  <p>数据范围：<strong>{html.escape(str(summary["data_start_utc"]))}</strong> 到 <strong>{html.escape(str(summary["data_end_utc"]))}</strong>，标的 <strong>{html.escape(INST_ID)}</strong>，周期 <strong>{html.escape(BAR)}</strong>。样本按 <strong>60% 训练 / 20% 验证 / 20% 测试</strong> 切分，费用按双边 taker <strong>0.036% + 0.036%</strong> 估算。</p>
</section>

<main class="wrap">
  <div class="grid grid-4">
    {kpi("最佳组合", f"斜率≤{best_config.slope_threshold:.4f}", f"止损 {best_config.stop_atr_mult:.1f} ATR")}
    {kpi("最佳测试 Avg R", f"{float(top_rows.iloc[0]['test_avg_r']):.3f}", f"测试 PF {float(top_rows.iloc[0]['test_profit_factor']):.2f}")}
    {kpi("稳定正收益组合", str(len(stable_rows)), "验证段和测试段同时为正，且测试 PF > 1")}
    {kpi("全样本交易数", str(int(best_metrics['trades'])), f"最佳组合全样本 PF {best_metrics['profit_factor']:.2f}")}
  </div>

  <h2>结论先看</h2>
  <div class="card answer">
    这组矩阵里，账面最优组合是 <span class="good">斜率阈值 ≤ {best_config.slope_threshold:.4f} + 止损 {best_config.stop_atr_mult:.1f} ATR</span>，但这次结果更该关注的是风险提示而不是“冠军参数”本身。<span class="bad">{validation_text}</span> {stability_text} 从平均表现看，<span class="good">更严格的负斜率过滤</span> 在测试段确实先有改善，但不是越严格越好，超过某个点后交易数会明显下滑，样本稳定性开始变差。<span class="warn">ATR 止损宽度方面，{float(strongest_stop):.1f} ATR 的测试段平均表现最好</span>；斜率维度上，均值最强的是 <span class="good">{float(strongest_slope):.4f}</span> 附近，而不是单纯追求最极端的负值。
  </div>

  <div class="grid grid-3">
    <div class="card">
      <h3>这次矩阵回答了什么</h3>
      <p>它重点回答的是：在同一套 EMA55 slope short 基线下，<strong>斜率过滤要不要更严格</strong>，以及 <strong>ATR 止损应该收紧还是放宽</strong>。所以这次没有混入新的止盈逻辑，尽量让结果只反映这两个参数维度。</p>
    </div>
    <div class="card">
      <h3>最值得关注的信号</h3>
      <p>如果一个组合在验证段和测试段都还能保持正的 Avg R，而且测试 PF 大于 1，它通常比只在全样本里好看的组合更值得信。{stability_text}</p>
    </div>
    <div class="card">
      <h3>这次的默认建议</h3>
      <p>如果你要先拿一组参数继续做后续研究，我建议先用 <strong>斜率 ≤ {best_config.slope_threshold:.4f}</strong>、<strong>止损 {best_config.stop_atr_mult:.1f} ATR</strong> 作为默认锚点，再往出场和仓位管理延展。</p>
    </div>
  </div>
  ...
  <div class="card">
    <div class="callout">
      ...
      1. 先看测试段 Avg R 和测试段 PF，确认有没有样本外优势。<br>
      2. 再看测试段交易数，避免只因为样本太少看起来很好。<br>
      3. 最后看全样本最大回撤 R，确认结果不是靠过大的回撤换出来的。
    </div>
    <p>从这 18 组里看，斜率从 <strong>-0.0005</strong> 往 <strong>-0.0008 / -0.0009</strong> 一带收紧时，通常会先提升信号质量；再继续往更深的负值压缩，交易数下降更明显，收益改善开始钝化。ATR 维度上，<strong>1.0 ATR</strong> 虽然反应快，但容易被正常波动洗掉；<strong>2.0 ATR</strong> 又会让单笔风险空间变宽，收益效率未必最好；因此中间档往往更平衡。</p>
  </div>
  ...
  <div class="card">
    {dataframe_table(
        ...
        [
            ...
            ("all_avg_r", "全样本 Avg R"),
            ("all_max_drawdown_r", "全样本最大回撤R"),
            ("score", "综合分"),
        ],
    )}
    <p class="note">综合分更偏向样本外表现，重点看测试段 Avg R、测试段 PF、验证段 Avg R 和交易数，不是简单按全样本收益排序。</p>
  </div>
  ...
  <h2>本次假设与说明</h2>
  <div class="card">
    <p>这份报告基于当前仓库里已有的 <strong>EMA55 slope short</strong> 研究逻辑，入场规则是 <code>(EMA55[t] - EMA55[t-1]) / EMA55[t] &lt;= 斜率阈值</code>，出场规则保持为 <strong>斜率转正平仓</strong>，止损为 <strong>entry + ATR14 * stop_atr_mult</strong>。</p>
    <p>也就是说，这次矩阵是一次 <strong>参数延展</strong>，不是切换策略核心逻辑。如果你说的 R001 另有一套未入库参数，我们后面可以再按那个版本复跑一轮。</p>
    <p class="note">数据行数：{len(df)} 根K线。最佳组合全样本指标：交易数 {int(best_metrics['trades'])}，胜率 {best_metrics['win_rate'] * 100:.1f}%，Avg R {best_metrics['avg_r']:.3f}，PF {best_metrics['profit_factor']:.2f}，最大回撤 {best_metrics['max_drawdown_r']:.2f}R。</p>
  </div>
</main>
</body>
</html>
...
def format_cell(column: str, value: object) -> str:
    ...
    if "threshold" in lower:
        return f"{number:.4f}"
    if "avg_r" in lower or "drawdown_r" in lower or "score" in lower:
        return f"{number:.3f}"

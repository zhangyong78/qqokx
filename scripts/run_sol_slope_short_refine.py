...
BASE_NAME = f"sol_slope_short_refine_{STAMP}"
CSV_PATH = REPORT_DIR / f"{BASE_NAME}.csv"
JSON_PATH = REPORT_DIR / f"{BASE_NAME}.json"
MD_PATH = REPORT_DIR / f"{BASE_NAME}.md"
LATEST_MD_PATH = PROJECT_REPORT_DIR / "sol_slope_short_refine_latest.md"
LATEST_CSV_PATH = PROJECT_REPORT_DIR / "sol_slope_short_refine_latest.csv"
LATEST_JSON_PATH = PROJECT_REPORT_DIR / "sol_slope_short_refine_latest.json"
...
def phase1_candidates() -> tuple[PhaseConfig, ...]:
    return (
        PhaseConfig(
            phase="line",
            candidate_id="line_ma20",
            label="MA20 / ATR14 / 2R保本 / 5R锁4R / step1",
            ema_type="ma",
            ema_period=20,
            trend_ema_type="ma",
            trend_ema_period=20,
        ...
        PhaseConfig(
            phase="line",
            candidate_id="line_ema55",
            label="EMA55 / ATR14 / 2R保本 / 5R锁4R / step1",
            ema_type="ema",
            ema_period=55,
            trend_ema_type="ema",
            trend_ema_period=55,
...
def overfit_risk_text(final_result: PhaseResult, phase1_best: PhaseResult, phase2_best: PhaseResult) -> str:
    risk_flags: list[str] = []
    ...
    if phase1_best.config.candidate_id != final_result.config.candidate_id and final_result.config.phase == "line":
        risk_flags.append("主导均线结论不稳定")
    if phase2_best.config.atr_period != final_result.config.atr_period and final_result.config.phase == "atr":
        risk_flags.append("ATR 口径结论不稳定")
    if not risk_flags:
...
def build_markdown(
    *,
    ...
    lines = [
        ...
        f"- 样本外：`2022-01-01 00:00:00 UTC` 之后按平仓时间统计",
    ...
    lines.extend(
        [
            ...
            f"- 过拟合判断：{overfit_risk_text(final_best, phase1_best, phase2_best)}",
            "",
            "## 年度样本外PnL",
            "",
            "| 年份 | PnL |",

...
def build_markdown_report(
    ...
    *,
    ...
    lines = [
        "# 1H 五币种 EMA21 + MA55 做多：MA55斜率温和变体对比",
        "",
        ...
        "- 方案A：最近 7 根 MA55 回归斜率 < 0 时，过滤做多。",
        "- 方案B：最近 5 根 MA55 回归斜率 / 当前 MA55 < -0.0005 时，过滤做多。",
    ...
    lines.append("- 如果两种过滤版都仍低于基线，说明问题不在过滤太激进，而在“过滤方向”本身未必适合这套 MA55 做多结构。")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    csv_path = REPORTS_DIR / f"ema21_ma55_long_slope_filter_variants_{timestamp}.csv"
    json_path = REPORTS_DIR / f"ema21_ma55_long_slope_filter_variants_{timestamp}.json"
    md_path = REPORTS_DIR / f"ema21_ma55_long_slope_filter_variants_{timestamp}.md"

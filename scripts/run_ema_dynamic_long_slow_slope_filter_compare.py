...
def build_markdown_report(
    ...
    *,
    ...
    lines = [
        "# 1H 五币种 EMA 动态委托做多：慢线回归斜率过滤对比",
        "",
        f"生成时间：{exported_at.strftime('%Y-%m-%d %H:%M:%S')}",
        "",
        "## 口径",
        "",
        "- 原版：EMA21 / 慢线 EMA55 / 挂单参考 EMA55 / ATR10 / SL=2ATR / 动态止盈 / 单笔风险 100。",
        f"- 新版：在原版基础上增加过滤，若最近 {TREND_SLOPE_LOOKBACK} 根慢线 EMA55 的线性回归斜率 < 0，则不产生新的做多委托。",
    ...
    if total_filtered_pnl > total_original_pnl:
        lines.append("- 新过滤条件在本轮样本中提升了总盈亏，可以继续观察它是否同时降低回撤和交易频率。")
    elif total_filtered_pnl < total_original_pnl:
        lines.append("- 新过滤条件在本轮样本中降低了总盈亏，更像是过度过滤，需要谨慎上线为默认条件。")
    else:
        lines.append("- 新过滤条件在本轮样本中没有改变总盈亏。对 EMA 来说，原策略已经要求收盘价站上 EMA55，通常也会让最近几根 EMA55 的整体回归斜率同步转正，所以该过滤和原条件仍有较强重叠。")
    lines.append("- 如果后续把趋势线类型切换成 MA，这个过滤可能会更有影响，因为 MA 可能出现“收盘在均线上方但均线本身仍下行”的情况。")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    csv_path = REPORTS_DIR / f"ema_dynamic_long_slow_slope_filter_compare_{timestamp}.csv"
    json_path = REPORTS_DIR / f"ema_dynamic_long_slow_slope_filter_compare_{timestamp}.json"
    md_path = REPORTS_DIR / f"ema_dynamic_long_slow_slope_filter_compare_{timestamp}.md"

...
class ExitVariant:
    ...
    ladder_start_r: float = 3.0
    ladder_lock_offset_r: float = 1.0
    ladder_step_r: float = 1.0
    ema_trail_start_r: float = 2.0
    pullback_start_r: float = 4.0
...
VARIANTS = (
    ExitVariant(
        key="baseline_ladder",
        label="原策略逐级锁盈",
        note="2R 先保本；3R 起每前进 1R，把止损推进到 trigger-1R。这个版本最积极。",
        kind="ladder",
        break_even_r=2.0,
        ladder_start_r=3.0,
        ladder_lock_offset_r=1.0,
        ladder_step_r=1.0,
    ),
    ExitVariant(
        key="delayed_4r_ladder",
        label="4R后才开始锁盈",
        note="2R 只保本，4R 才锁 1R，之后每 1R 推进一次。给趋势更大呼吸空间。",
        kind="ladder",
        break_even_r=2.0,
        ladder_start_r=4.0,
        ladder_lock_offset_r=3.0,
        ladder_step_r=1.0,
    ),
    ExitVariant(
        key="wide_2r_ladder",
        label="每2R推进一次",
        note="2R 保本，4R 锁 1R，6R 锁 3R，8R 锁 5R；减少被普通反抽扫出。",
        kind="ladder",
        break_even_r=2.0,
        ladder_start_r=4.0,
        ladder_lock_offset_r=3.0,
    ...
    ExitVariant(
        key="ema21_after_2r",
        label="2R后EMA21跟踪",
        note="2R 先保本，随后止损跟随 EMA21 下移；价格反抽触及 EMA21 附近再出。",
        kind="ema21_trail",
        break_even_r=2.0,
        ema_trail_start_r=2.0,
    ),
    ExitVariant(
        key="ema21_after_4r",
        label="4R后EMA21跟踪",
        note="2R 只保本，到 4R 后才启动 EMA21 跟踪，试图同时保留安全垫和趋势弹性。",
        kind="ema21_trail",
        break_even_r=2.0,
        ema_trail_start_r=4.0,
    ),
    ExitVariant(
        key="pullback_35_after_4r",
        label="4R后回撤35%止盈",
        note="2R 保本；最大浮盈达到 4R 后，如果回吐超过 35%，保留约 65% 已有浮盈。",
        kind="profit_pullback",
        break_even_r=2.0,
        pullback_start_r=4.0,
        keep_profit_ratio=0.65,
    ),
    ExitVariant(
        key="pullback_50_after_4r",
        label="4R后回撤50%止盈",
        note="2R 保本；最大浮盈达到 4R 后，允许回吐 50%，比 35% 版本更能扛波动。",
        kind="profit_pullback",
        break_even_r=2.0,
        pullback_start_r=4.0,
        keep_profit_ratio=0.50,
    ),
)
...
def main() -> None:
    ...
    basename = f"ema55_slope_short_exit_variants_5coins_10u_{stamp}"
...
def simulate_trades(df: pd.DataFrame, variant: ExitVariant) -> pd.DataFrame:
    ...
    for index in range(start_index, len(df)):
        ...
        position = {
            ...
            "next_ladder_trigger_r": variant.break_even_r,
            "break_even_done": 0.0,
            "ema_trail_active": 0.0,
            "pullback_active": 0.0,
            "max_favorable_r": 0.0,
            "entry_ema21": ema21,
            "entry_ema55": ema55,
    ...
    *,
    ...
    for start, end in zip(path, path[1:]):
        if end > start:
            ...
            advance_exit_stop(position, variant, favorable_price=end, ema21=ema21)
    return None
...
def advance_exit_stop(
    ...
    *,
    ...
    if favorable_r >= variant.break_even_r and not bool(position["break_even_done"]):
        update_stop(position, entry - fee_offset, "break_even_stop")
        position["break_even_done"] = 1.0

    if variant.kind == "ladder":
        while float(position["next_ladder_trigger_r"]) <= favorable_r:
            trigger_r = float(position["next_ladder_trigger_r"])
            if trigger_r <= variant.break_even_r:
                locked_r = 0.0
                reason = "break_even_stop"
            else:
                locked_r = max(trigger_r - variant.ladder_lock_offset_r, 0.0)
                reason = f"locked_{format_r(locked_r)}r_stop"
            update_stop(position, entry - risk * locked_r - fee_offset, reason)
            position["next_ladder_trigger_r"] = trigger_r + variant.ladder_step_r
        return

    if variant.kind == "ema21_trail" and favorable_r >= variant.ema_trail_start_r:
        position["ema_trail_active"] = 1.0
        update_stop(position, ema21 - fee_offset, "ema21_trail_stop")
        return

    if variant.kind == "profit_pullback" and float(position["max_favorable_r"]) >= variant.pullback_start_r:
        position["pullback_active"] = 1.0
...
def build_summary_frame(trades_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for variant in VARIANTS:
        ...
        pnl_values = []
        for _, coin_group in group.groupby("coin"):
            pnl_values.append(compute_metrics(split_trades(coin_group, "test"))["total_pnl_u"])
        rows.append(
            {
                "mode_key": variant.key,
                "mode_label": variant.label,
...
def build_payload(
    ...
    return {
        ...
        "assumptions": {
            "entry": "EMA55 slope short baseline; no same-bar or rebound re-entry block.",
...
<head>
  ...
  <title>EMA55 斜率做空出场优化报告</title>
  <style>
    :root {{
      --bg:#f4f7f3; --panel:#ffffff; --ink:#172033; --muted:#667085; --line:rgba(23,32,51,.10);
      --accent:#275c4c; --accent2:#b85c38; --shadow:0 18px 38px rgba(31,41,55,.08);
    }}
    * {{ box-sizing:border-box; }}
    body {{
      margin:0; color:var(--ink); font-family:"Microsoft YaHei","Segoe UI",sans-serif;
      background:
        radial-gradient(circle at 10% 0%, rgba(39,92,76,.12), transparent 30%),
        radial-gradient(circle at 90% 8%, rgba(184,92,56,.10), transparent 28%),
        linear-gradient(180deg,#fbfcf8 0%,var(--bg) 100%);
    }}
    .wrap {{ width:min(1240px,calc(100vw - 30px)); margin:0 auto; padding:28px 0 56px; }}
    .hero {{ border-radius:28px; padding:30px; color:white; background:linear-gradient(135deg,#275c4c,#303947); box-shadow:var(--shadow); }}
    .hero h1 {{ margin:10px 0 8px; font-size:34px; line-height:1.08; }}
    .hero p {{ margin:8px 0 0; max-width:940px; line-height:1.75; color:rgba(255,255,255,.9); }}
    ...
    .s,.note {{ color:var(--muted); line-height:1.65; font-size:13px; margin-top:8px; }}
    ...
    .mode-card {{ border:1px solid var(--line); border-radius:16px; padding:16px; background:#f9fbf7; }}
    img {{ width:100%; border-radius:16px; border:1px solid var(--line); background:#fff; }}
    table {{ width:100%; border-collapse:collapse; font-size:13px; }}
    th,td {{ text-align:left; padding:10px 8px; border-bottom:1px solid rgba(23,32,51,.08); white-space:nowrap; }}
    th {{ color:var(--muted); background:#f7faf5; position:sticky; top:0; }} .scroll {{ overflow:auto; }}
...
<body>
  <div class="wrap">
    <section class="hero">
      <div class="eyebrow">Slope Short / Exit Management</div>
      <h1>EMA55 斜率做空出场优化矩阵</h1>
      <p>这次不再修“再入场”，而是直接测试你的核心怀疑：做空单如果已经走出一定盈利空间，是否不该太急动态止盈。入场、风控、币种、风险金全部固定，只替换出场管理。</p>
    </section>
    <section class="grid">
      ...
      <div class="card stat"><div class="k">测试段回撤变化</div><div class="v">{improve_dd:+.1f}U</div><div class="s">负值表示比原策略更稳。</div></div>
      ...
      <div class="card wide"><h2>测试口径</h2><ul>
        ...
        <li>风控：ATR14 * {ATR_STOP_MULTIPLIER:.1f}，每笔固定风险 {RISK_PER_TRADE_U:.1f}U，taker {TAKER_FEE_RATE * 100:.3f}%</li>
        <li>样本切分：每币种 60% 训练、20% 验证、20% 测试；未加入同根禁开或 EMA 反弹再入场。</li>
      </ul></div>
      <div class="card wide"><h2>出场版本</h2><div class="mode-grid">{mode_cards}</div></div>
      <div class="card full"><h2>数据覆盖</h2><ul>{data_lines}</ul></div>

      <div class="card wide"><h2>全样本总盈亏</h2><img src="data:image/png;base64,{render_bar(summary_frame, "all_pnl_u", "全样本总盈亏", "#275c4c")}" alt="all_pnl"></div>
      <div class="card wide"><h2>测试段总盈亏</h2><img src="data:image/png;base64,{render_bar(summary_frame, "test_pnl_u", "测试段总盈亏", "#b85c38")}" alt="test_pnl"></div>
      ...
      <div class="card full"><h2>模式总表</h2><div class="note">默认按综合评分排序。重点同时看收益、回撤、PF、持仓时长和平均最大浮盈，判断是否真的“让利润奔跑”。</div><div class="scroll" style="margin-top:12px;">{summary_table}</div></div>
      ...
      <div class="card wide"><h2>各币种全样本 PnL</h2><img src="data:image/png;base64,{render_coin_heatmap(coin_frame, "all_pnl_u", "各币种全样本 PnL")}" alt="coin_all"></div>

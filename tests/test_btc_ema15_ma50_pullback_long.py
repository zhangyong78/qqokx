...
from okx_quant.strategy_catalog import STRATEGY_BTC_EMA15_MA50_PULLBACK_LONG_ID
from okx_quant.strategies.btc_ema15_ma50_pullback_long import (
    PullbackCandidate,
    evaluate_btc_ema15_ma50_pullback_long_signal,
    is_cross_up,
...
class BtcEma15Ma50PullbackLongTest(TestCase):
    ...
    def test_is_cross_up_uses_previous_and_current_values(self) -> None:
        fast = [Decimal("99"), Decimal("100"), Decimal("101")]
        slow = [Decimal("100"), Decimal("100"), Decimal("100")]

        self.assertFalse(is_cross_up(fast, slow, 1))
        self.assertTrue(is_cross_up(fast, slow, 2))

    def test_scan_candidates_detects_first_pullback_after_cross(self) -> None:
        candles = _candles(12)
        candles[7] = Candle(candles[7].ts, Decimal("100"), Decimal("104"), Decimal("99"), Decimal("103"), Decimal("1"), True)
        config = replace(_config(), ema_period=5, trend_ema_period=6, atr_period=5, cross_window_bars=5)
        fast_values = [None, None, None, None, Decimal("99"), Decimal("99"), Decimal("101"), Decimal("102"), Decimal("103"), Decimal("104"), Decimal("105"), Decimal("106")]
        ...
        with patch("okx_quant.strategies.btc_ema15_ma50_pullback_long.moving_average", side_effect=[fast_values, slow_values]), patch(
            "okx_quant.strategies.btc_ema15_ma50_pullback_long.atr",
            return_value=atr_values,
        ):
            candidates = scan_btc_ema15_ma50_pullback_long_candidates(candles, config)
    ...
    def test_scan_candidates_invalidates_when_cross_window_expires(self) -> None:
        candles = _candles(12)
        candles[9] = Candle(candles[9].ts, Decimal("100"), Decimal("103"), Decimal("99"), Decimal("102"), Decimal("1"), True)
    ...
    def test_scan_candidates_invalidates_when_fast_falls_back_below_slow(self) -> None:
        candles = _candles(12)
        candles[8] = Candle(candles[8].ts, Decimal("100"), Decimal("103"), Decimal("99"), Decimal("102"), Decimal("1"), True)
        config = replace(_config(), ema_period=5, trend_ema_period=6, atr_period=5, cross_window_bars=5)
        fast_values = [None, None, None, None, Decimal("99"), Decimal("99"), Decimal("101"), Decimal("99"), Decimal("99"), Decimal("99"), Decimal("99"), Decimal("99")]
        slow_values = [None, None, None, None, Decimal("100"), Decimal("100"), Decimal("100"), Decimal("100"), Decimal("100"), Decimal("100"), Decimal("100"), Decimal("100")]
        atr_values = [None, None, None, None, Decimal("10"), Decimal("10"), Decimal("10"), Decimal("10"), Decimal("10"), Decimal("10"), Decimal("10"), Decimal("10")]
    ...
    def test_evaluate_signal_respects_pullback_index_limit(self) -> None:
        ...
        with patch(
            "okx_quant.strategies.btc_ema15_ma50_pullback_long.scan_btc_ema15_ma50_pullback_long_candidates",
            return_value=[candidate],
        ):
            decision = evaluate_btc_ema15_ma50_pullback_long_signal(candles, config)
    ...
    def test_backtest_enters_on_next_open_and_applies_fixed_risk_with_costs(self) -> None:
        ...
        with patch("okx_quant.backtest.scan_btc_ema15_ma50_pullback_long_candidates", return_value=[candidate]):
            result = run_backtest(
                client,
                _config(),

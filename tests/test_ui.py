...
class StrategyDuplicateLaunchGuardTest(TestCase):
    ...
    def test_upsert_session_row_marks_duplicate_conflict_tag(self) -> None:
        ...
        self.assertEqual(app.session_tree.rows["S01"]["values"][10], "1H")
        self.assertEqual(app.session_tree.rows["S01"]["values"][12], "10")
        self.assertEqual(app.session_tree.rows["S01"]["values"][18], "-")
        self.assertEqual(app.session_tree.rows["S01"]["values"][19], "-")
        self.assertEqual(app.session_tree.rows["S01"]["values"][21], "+1.25")
...
class StrategyTradeTrackingTest(TestCase):
    ...
    def test_recover_session_prefers_local_trade_management_mode_over_exchange_config(self) -> None:
        session = self._make_session()
        session.strategy_id = STRATEGY_EMA55_SLOPE_SHORT_ID
        session.symbol = "SOL-USDT-SWAP"
        session.config = StrategyConfig(

...
from report_v5 import write_v5_research_report
...
def main() -> None:
    ...
    v5_config = dict(config)
    v5_config["results_dir"] = str(config["v5_results_dir"])
    results_path = results_dir(v5_config)
    ...
    save_v5_trade_charts(aligned_entry, comparison, trades, results_path / "trade_charts", int(config.get("v5_trade_chart_limit", 60)))

    write_v5_research_report(
    ...
    print(f"V5 research complete: {results_path}")


def save_v5_trade_charts(
    frame: pd.DataFrame,
